var entity_edit_model  = {
    ...
    menu:null,
    url_index:0,
    url:erp_api_service.EntRelItemItem,
    top_menu:null,
    checked_record:[],
    default_pages_size:15,//默认页面大小
    pages_size:[30,50,80,100],//定义分页时 页面的大小级别
    dispaly_columns:
    	[
              { display: '序号', name: 'id', width: 30, type: 'int' },
              {
                  display: '父', name: 'p_item_number', width: 100,
                  editor: { type: 'text', height: 60 }
              },
              {
            	  display: '子', name: 'c_item_number', width: 100,
            	  editor: { type: 'text', height: 60 }
              },
              {
            	  display: '数量', name: 'count', width: 100,
            	  editor: { type: 'text', height: 60 }
              },
              {
...
function f_initGrid() {
    ...
    entity_edit_model .manager = $("#material_edit").ligerGrid({
        ...
        groupColumnName: 'p_item_number', 
        groupRender: function (city,groupdata)
        {
            return  city + '(子件数量=' + groupdata.length + ')';
        },
        isScroll: false,
        frozen: false,

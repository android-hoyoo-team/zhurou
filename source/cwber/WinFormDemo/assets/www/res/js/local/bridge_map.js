...
var bridge_map = {
    ...
    */
    ...
    folder_browser_dialog: function (param) {
        if (use_bridge_tag) {
            res = window.CallCSharpMethod("folder_browser_dialog", JSON.stringify(param));
     ...
     * filter_index
     */
    open_file_dialog: function (param) {
        if (use_bridge_tag) {
            var res = window.CallCSharpMethod("open_file_dialog", JSON.stringify(param));
    ...
    */
    get_file_size: function (file_name) {
        if (use_bridge_tag) {
            var res = window.CallCSharpMethod("get_file_size", file_name);
    ...
    /**
        获取当前系统窗口的大小参数
    */
    get_system_parameters: function (param) {
        if (use_bridge_tag) {
            if (!param)
                param = { null: "" };
            var res = window.CallCSharpMethod("get_system_parameters", JSON.stringify(param));
            //var res= window.external.get_file_size(file_name);
            if (res) {
                return JSON.parse(res)
            }

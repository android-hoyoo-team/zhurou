							...
							.get_default_grid_option_for_url({
								...
								detail : {
									...
									onShowDetail : function(row, detailPanel,callback) {
							        	...
							        	var _erp_bom_edit=new erp_drawing_edit({div:grid,item_number:row.number});
//										current_id=row.id;
//										var grid = document
//												.createElement('div');
//										$(detailPanel).append(grid);
...
//											                                    	 	technology_rank:arguments[0].newdata.technology_rank,
...
//											                             else {
//											                                 param = {
//											                                     url: erp_api_service.EntRelItemDrawing[0]+ arguments[0].record.id,
//											                                     method: "PATCH",
...
function deleteTechnologyById(id,current_id) {
    if (confirm('确定删除?')) {
    	if(ERPUtils.deleteEntityById(id,erp_api_service.EntRelItemDrawing[0]))

...
(function ($)
{
    ...
    $.ligerDefaults.Grid.formatters['date'] = function (value, column)
    {
    	...
    	{
        	var format = column.format || this.options.dateFormat || "yyyy-MM-dd";
            return getFormatDate(new Date(value*1000), format);
    		//console.info(date);
    		//date=new Date(date);
    	}
        else
        {
        	...
        	if (isNaN(myDate))  { 
        		try{
        			var delimCahar = value.indexOf('/')!=-1?'/':'-'; 
        			var arys= value.split(" "); 
        			var ds=null;
        			var ts=null;
        			if(arys.length>1)
        			{
        				ds=arys[0].split(delimCahar);
        				ts=arys[1].split(":");
        			}
        			else
        			{
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
          ...
          //  gridhtmlarr.push("                <div class='l-bar-group  l-bar-message'><span class='l-bar-text'></span></div>");
            ...
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            //
            gridhtmlarr.push("                <div class=' l-bar-message1'><span class='l-bar-text'></span></div>");
        ...
        loadServerData: function (param, clause)
        {
            ...
            var ajaxOptions = {
                type: p.method,
                url: p.urlFilter&&typeof(p.urlFilter)=="function"?p.urlFilter.call(this,p.url,this):p.url,
                data: p.paramFilter&&typeof(p.paramFilter)=="function"?p.paramFilter.call(this,param,this):param,
                async: p.async,
                dataType: 'json',
                beforeSend: function ()
                {
        ...
        submitEdit: function (rowParm)
        {
            ...
            else
            {
                ...
                for (var columnid in g.editors[rowdata['__id']])
                {
                    ...
                    if (column.name)
                    {
                    	var d=o.editor.getValue(o.input, o.editParm);
                    	
                    	if(column.type=='int'||column.type=='Number')
                    		newdata[column.name] =d==""?null:Number(d);
                    	else
